...
class BlackViewController: UIViewController,MFMailComposeViewControllerDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if memoryTag.string(forKey: "flag") == "ON"{
            titlecontent.text = "纽约领事馆"
            Phone1.text = "使馆电话:"
            Phone2.text = "求助热线:"
            Email.text = "电子邮件:"
            Note.text = "(非工作时间及公共假日期间，仅供紧急联络使用)"
            Address.text = "使馆地址: 520 12th Avenue, \nNew York, NY, 10036"
        }
        else{
            titlecontent.text = "Chinese Embassy in New York"
            Phone1.text = "Embassy call:"
            Phone2.text = "Call for help:"
            Email.text = "Email:"
            Note.text = "(Only for Emergency!)"
            Address.text = "Embassy Address: 520 12th Avenue, \nNew York, NY, 10036"
            
        }
        background.frame = CGRect(x: 0, y: 0, width: fullScreenSize.width, height: fullScreenSize.height)
        background.image = UIImage(named:"Action4.png")
    ...
    func configuredMailComposeViewController() -> MFMailComposeViewController {
        ...
        //设置邮件地址、主题及正文
        mailComposeVC.setToRecipients(["cnnyconsulate@gmail.com"])
