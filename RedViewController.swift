...
class RedViewController: UIViewController,MFMailComposeViewControllerDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if memoryTag.string(forKey: "flag") == "ON"{
            titlecontent.text = "中国驻美国大使馆"
            Phone1.text = "使馆电话:"
            Phone2.text = "值班手机:"
            Email.text = "电子邮件:"
            Note.text = "(非工作时间及公共假日期间，仅供紧急联络使用)"
            Address.text = "使馆地址: 3505 International Place\nNW, Washington,\nD.C. 20008"
        }
        else{
            titlecontent.text = "Chinese Embassy in US"
            Phone1.text = "Embassy call:"
            Phone2.text = "On duty call:"
            Email.text = "Email:"
            Note.text = "(Only for Emergency!)"
            Address.text = "Embassy Address: 3505 International Place\nNW, Washington,\nD.C. 20008"

        }
        background.frame = CGRect(x: 0, y: 0, width: fullScreenSize.width, height: fullScreenSize.height)
        background.image = UIImage(named:"Action3.png")
    ...
    func configuredMailComposeViewController() -> MFMailComposeViewController {
        ...
        //设置邮件地址、主题及正文
        mailComposeVC.setToRecipients(["chinaembpress_us@mfa.gov.cn"])
